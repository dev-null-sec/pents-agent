...
| vision-reviewer | 视觉复核子代理：只读取截图，判断验证码、WAF 页面、视觉布局、遮挡和脱敏需求 |
...
当页面状态必须依赖视觉判断时，主 agent 应主动调用 `vision-reviewer` 子代理，而不是等待用户在任务卡中额外提醒。
...
| 验证码、Turnstile、canvas、图片化页面、遮挡、脱敏 | vision-reviewer | 只回答窄问题，输出结构化 JSON |
...
主 agent 在以下场景应自动调用 `vision-reviewer`：
...
- 给 `vision-reviewer` 的问题必须窄，例如“这张截图里是否出现 Turnstile？”。
- 只传截图路径、问题和必要上下文，不让视觉子代理访问网络或操作浏览器。
- 推荐子代理模板使用 `model: haiku`；实际是否能读图必须通过 canary 验证，不能仅凭模型名假设。
- 如果视觉子代理返回 `can_read_image=false` 或 `blocker=model_no_image_input`，记录 blocker，不猜测图片内容。
...
| 视觉复核 | `runs/Rxxx/outputs/browser/visual-reviews/*.json` | vision-reviewer 对截图的结构化判断 |
